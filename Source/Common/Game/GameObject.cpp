...
void GameObject::update(double aDelta)
{
	...
	{
		m_UpdateForce = false;
		m_Body->ApplyForce(m_Force, m_ForcePoint, true);
	}
	...
	{
		m_UpdateLinearImpulse = false;
		m_Body->ApplyLinearImpulse(m_LinearImpulse, m_LinearImpulsePoint, true);
	}

	if (m_UpdateAngularImpulse == true)
	{
		m_UpdateAngularImpulse = false;
		m_Body->ApplyAngularImpulse(m_AngularImpulse, true);
	}
