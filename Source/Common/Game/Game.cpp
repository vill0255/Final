...
void Game::load()
{
	...
	case GameLoadStepPlatforms:
	{
		...
		float y = getScreenHeight() * 0.9f;
		addGameObject(new StaticPlatform(b2Helper::screenSpaceToBox2dSpace(x, y), b2Vec2(0.5f, 2.0f)));

		x = getScreenWidth() * 0.6f;
		y = getScreenHeight() * 0.42f;
		addGameObject(new StaticPlatform(b2Helper::screenSpaceToBox2dSpace(x, y), b2Vec2(0.5f, 2.0f)));

		x = getScreenWidth() * 0.85f;
		y = getScreenHeight() * 0.9f;
		addGameObject(new StaticPlatform(b2Helper::screenSpaceToBox2dSpace(x, y), b2Vec2(2.0f, 0.5f), 135.0f));

		x = getScreenWidth() * 0.8f;
		y = getScreenHeight() * 0.05f;
		addGameObject(new StaticPlatform(b2Helper::screenSpaceToBox2dSpace(x, y), b2Vec2(2.0f, 0.5f), -135.0f));

		x = getScreenWidth() * 0.38f;
		y = getScreenHeight() * 0.95f;
		addGameObject(new StaticPlatform(b2Helper::screenSpaceToBox2dSpace(x, y), b2Vec2(2.0f, 0.5f), -135.0f));

		x = getScreenWidth() * 0.28f;
		y = getScreenHeight() * 0.5f;
		addGameObject(new StaticPlatform(b2Helper::screenSpaceToBox2dSpace(x, y), b2Vec2(2.0f, 0.5f), 0.0f, 1.5f));

		x = getScreenWidth() * 0.01f;
		y = getScreenHeight() * 0.5f;
		addGameObject(new StaticPlatform(b2Helper::screenSpaceToBox2dSpace(x, y), b2Vec2(2.0f, 0.5f), -55.0f, 0.2f));

		x = getScreenWidth() * 0.2f;
		y = getScreenHeight() * 0.25f;
		float minX = getScreenWidth() * 0.05f;
		float maxX = getScreenWidth() * 0.4f;
	...
	{
		float x = getScreenWidth() * 0.1125f;
		float y = getScreenHeight() * 0.15f;
